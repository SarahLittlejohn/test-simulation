...
from find_switching_rate import find_static_switching_rate_noisy_series
import numpy as np
import pandas as pd
...
def bootstrap_model_switching_rate(series, n_bootstrap=1000, segment_length=None):
    ...
    for _ in range(n_bootstrap):
        ...
        rate_estimate = find_static_switching_rate_noisy_series(bootstrap_sample)
...
for x in sample:
    ...
    mean_rate, std_rate = bootstrap_model_switching_rate(series, segment_length=1000)
    switching_rate = find_static_switching_rate_noisy_series(series)
