...
for i, (row, dist) in enumerate(zip(gaussian_matrix[1:], adjusted_d)):
    ...
    if min_rate < global_min_rate:
        global_min_rate = min_rate
...
if global_min_d is not None:
    axs[1].scatter(global_min_d, 0, color='red', marker='x', s=100, linewidths=2, label=f'Extracted d_impact = {global_min_d}')
...
print(f"Extracted d_impact: {global_min_d}")
print(f"Minimum Switching Rate: {global_min_rate:.4f}")
